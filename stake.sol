...
contract StakingContract is Ownable {
    ...
    constructor(address _mainToken, address _rewardToken) {
        ...
        rewardPerSecond = revenue * 1e18 / (850000000 * 2629248);
    ...
    function stake(uint256 amount) external {
        ...
        require(mainToken.balanceOf(msg.sender) >= amount, "Insufficient balance");
    ...
    function withdraw() external {
        ...
        mainToken.transfer(msg.sender, stakedAmount);
    ...
    function setRevenue(uint256 _revenue) external onlyOwner {
        revenue = _revenue;
        rewardPerSecond = revenue / 850000000 / SECONDS_IN_A_MONTH;
    }
    ...
    function distributeReward() external onlyOwner {
        ...
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            // Reset user's reward and timer
            stakingData[staker].lastUpdateTime = block.timestamp;
            stakingData[staker].rewardEarned = 0;

            require(rewardToken.transfer(staker, reward), "Reward transfer failed");
        }

        if (totalReward > rewardAmount) {
            require(rewardToken.transfer(msg.sender, totalReward - rewardAmount), "Excess reward transfer to owner failed");
        }
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(tokenAddress != address(mainToken), "Cannot withdraw the staking token");
